                    ...
                    <div class="relative overflow-x-auto">
                        @if($finance_facilities->isNotEmpty())
                            ...
                            <table class="min-w-max w-full table-auto">
                                ...
                                @foreach ($finance_facilities as $facility)
                                    <tr class="border-b border-gray-200 hover:bg-gray-100">
                                        <td class="py-1 px-2 text-center">
                                            {{ $loop->iteration }}
                                        </td>
                                        <td class="py-1 px-2 text-center">
                                            {{ $facility->institution_name }}
                                        </td>
                                        <td class="py-1 px-2 text-center">
                                        ...
                                        <td class="py-1 px-2 text-center">
                                            ...
                                            @if($facility->is_authorize == "No")
                                                @can('Authorizer')
                                                    <form action="{{ route('finance_facility.authorized', [$facility->borrower_id, $facility->id]) }}" method="post" class="inline-block" onsubmit="return confirm('Do you really want to authorized this record?');">
                                                        @csrf
                                                        @method('PUT')
                                                ...
                                                @can('Inputter')
                                                    ...
                                                    <form action="{{ route('finance_facility.destroy', [$facility->borrower_id, $facility->id]) }}" method="post" class="inline-block" onsubmit="return confirm('Do you really want to delete the record?');">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="inline-flex">
                                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
