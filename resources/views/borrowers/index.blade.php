...
{{--                            <option value="{{ $bg->id }}" {{ request( 'filter.blood_group_id')==$ bg->id ? 'selected' : '' }} > {{ $bg->name }}--}}
{{--                            </option>--}}
{{--                        @endforeach--}}
                    </select>
                </div>
...
{{--                            <option value="{{ $branch->id }}" {{ request( 'filter.branch_id')==$ branch->id ? 'selected' : '' }}> {{ $branch->branch_custom_code }}--}}
{{--                            </option>--}}
{{--                        @endforeach--}}
                    </select>
                </div>
...
{{--                            <option value="{{ $st->id }}" @if(!empty(request( 'filter')[ 'latestStatus.status_id'])) {{ request( "filter")[ 'latestStatus.status_id']==$ st->id ? 'selected' : '' }} @endif > {{ $st->name }}--}}
{{--                            </option>--}}
{{--                        @endforeach--}}
                    </select>
                </div>
...
{{--                                        {{ $student->branch?->id . '-' .$student->id }} --}}{{-- /{{ \Carbon\Carbon::parse($student->sessionYear->session->start_year)->format('y-') . \Carbon\Carbon::parse($student->sessionYear->session->end_year)->format('y') }}--}}
{{--                                    </td>--}}
...
{{--                                            <span class="bg-red-100 text-black text-xs me-2 px-2.5 py-0.5 rounded">{{ $cert->certificate->name }}</span> @if($student->applied_for_certificates->count() > 1 && !$loop->last)--}}
{{--                                                <br> @endif @endforeach--}}
{{--                                    </td>--}}
...
{{--                                        <a href="{{ route('student.profile-print', $student->id) }}" class="inline-flex items-center px-0.5 py-0.5 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">--}}
{{--                                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6">--}}
